...
class StaticChecker(Visitor):
    ...
    def visitBinExpr(self, ast, o):
        ...
        if (type(ltype) in [ParamSymbol, FuncSymbol, VarSymbol] and not type(rtype) in [AutoType, ParamSymbol, FuncSymbol]) or (type(rtype) in [ParamSymbol, FuncSymbol, VarSymbol] and not type(ltype) in [AutoType, ParamSymbol, FuncSymbol]):
            self.inferTypeForExpr(ast, ltype, ast.left, rtype, ast.right, o)
            if type(ltype) in [ParamSymbol, FuncSymbol, VarSymbol] and not type(rtype) in [AutoType, ParamSymbol, FuncSymbol]:
                ltype = rtype
            else:
                rtype = ltype
